...
int tracing (symbol_t name) {
  int i = 0;
  while (i < lengthof(TraceBuf)) {
    if (TraceBuf[i].fn == name) return i+1;
    i++;
  }
  return 0;
}
...
void untrace (symbol_t name) {
  int i = 0;
  while (i < lengthof(TraceBuf)) {
    if (TraceBuf[i].fn == name) { TraceBuf[i].fn = 0; return; }
    i++;
  }
...
int eq (object *arg1, object *arg2) {
  if (arg1 == arg2) return 1;  // Same object
  if ((arg1 == nil) || (arg2 == nil)) return 0;  // Not both values
  if (arg1->cdr != arg2->cdr) return 0;  // Different values
  if (symbolp(arg1) && symbolp(arg2)) return 1;  // Same symbol
  if (integerp(arg1) && integerp(arg2)) return 1;  // Same integer
  if (floatp(arg1) && floatp(arg2)) return 1; // Same float
  if (characterp(arg1) && characterp(arg2)) return 1;  // Same character
  return 0;
}
...
object *closure (int tc, object *fname, object *state, object *function, object *args, object **env) {
  ...
  if (trace) {
    indent(TraceBuf[trace-1].depth<<1, pserial);
    pint(TraceBuf[trace-1].depth++, pserial);
...
object *sp_trace (object *args, object *env) {
  ...
  int i = 0;
  while (i < lengthof(TraceBuf)) {
    if (TraceBuf[i].fn != 0) args = cons(symbol(TraceBuf[i].fn), args);
    i++;
  }
  return args;
}
...
struct timespec *ts_diff(struct timespec *td, const struct timespec *te, const struct timespec *ts)
{
  if (te->tv_nsec < ts->tv_nsec) {
    td->tv_sec = te->tv_sec - ts->tv_sec;
    td->tv_nsec = te->tv_nsec - ts->tv_nsec;
  } else {
    td->tv_sec = te->tv_sec - ts->tv_sec;
    td->tv_nsec = te->tv_nsec - ts->tv_nsec;
  }
...
unsigned long ts_to_ms(const struct timespec *t)
{
  unsigned long msec;

  msec = 0UL;
  if (t) {
    msec = (t->tv_sec * 1000) + (t->tv_nsec / 1000000);
  }
  return msec;
}

unsigned long ts_to_us(const struct timespec *t)
{
  unsigned long usec;
...
object *sp_withspi (object *args, object *env) {
  (void) args, (void) env;
  error(PSTR("with-spi not supported"));
  return nil;
}
...
object *fn_abs (object *args, object *env) {
  (void) env;
  object *arg = first(args);
  if (floatp(arg)) return makefloat(fabs(fromfloat(arg)));
  else {
    int result = integer(arg);
    if (result == INT_MIN) return makefloat(fabs((float)integer(arg)));
    else return number(fabs(result));
  }
}
...
int stringcompare (object *args, int lt, int gt, int eq) {
  object *arg1 = first(args);
  object *arg2 = second(args);
...
object *fn_stringeq (object *args, object *env) {
  (void) env;
  return stringcompare(args, 0, 0, 1) ? tee : nil;
}

object *fn_stringless (object *args, object *env) {
  (void) env;
  return stringcompare(args, 1, 0, 0) ? tee : nil;
}

object *fn_stringgreater (object *args, object *env) {
  (void) env;
  return stringcompare(args, 0, 1, 0) ? tee : nil;
}
...
object *fn_logbitp (object *args, object *env) {
  (void) env;
  unsigned int index = integer(first(args));
  unsigned int value = integer(second(args));
  return (value & (1<<index)) ? tee : nil;
}
...
object *fn_gc (object *obj, object *env) {
  ...
  elapsed = ts_to_us(ts_diff(&td, &tn, &ts));
...
object *fn_pinmode (object *args, object *env) {
  (void) args, (void) env;
  error(PSTR("pinmode not supported"));
  return nil;
}

object *fn_digitalread (object *args, object *env) {
  (void) args, (void) env;
  error(PSTR("digitalread not supported"));
  return nil;
}

object *fn_digitalwrite (object *args, object *env) {
  (void) args, (void) env;
  error(PSTR("digitalwrite not supported"));
  return nil;
}
...
object *fn_delay (object *args, object *env) {
  ...
  msec = integer(arg1);
  td.tv_sec = msec / 1000;
  td.tv_nsec = (msec - td.tv_sec*1000) / 1000000;
  nanosleep(&td, NULL);
  return arg1;
}

object *fn_millis (object *args, object *env) {
  struct timespec tn, td;
  (void) args, (void) env;
  clock_gettime(CLOCK_MONOTONIC, &tn);
  return number(ts_to_ms(ts_diff(&td, &Epoch, &tn)));
}
...
void supersub (object *form, int lm, int super, pfun_t pfun) {
  ...
  if (symbolp(arg)) {
    ...
    else for (int i=0; i<lengthof(ppspecial); i++) {
...
object *nextitem (gfun_t gfun) {
  ...
  while(!isspace(ch) && ch != ')' && ch != '(' && index < bufmax) {
    ...
    if (base == 10 && ch == '.' && !isexponent) {
      ...
      if (!isfloat) { isfloat = 1; fresult = result; }
      isexponent = 1;
...
void loop (void) {
  ...
  td.tv_sec = 0;
  td.tv_nsec = 1000000;
  nanosleep(&td, NULL); while (Serial.available()) Serial.read();
  for (int i=0; i<lengthof(TraceBuf); i++) TraceBuf[i].depth = 0;
  repl(NULL);
}
